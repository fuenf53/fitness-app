/** Chart.js line chart of one exercise's top set over time. Mirrors WeightChart.jsx. */
...
export default function StrengthChart({ history, isBodyweight }) {
  ...
  useEffect(() => {
    ...
    const data = history.map((h) => (isBodyweight ? h.reps : round1(kgToDisplay(h.weightKg, units))));
          ...
          tooltip: {
            ...
            callbacks: {
              label: (c) => {
                ...
                return isBodyweight
                  ? `${h.reps} reps`
                  : `${round1(kgToDisplay(h.weightKg, units))} ${unit} × ${h.reps}`;
              },
            },
          },
        },
  ...
  }, [history, units, theme, isBodyweight]);

  return (
    <div className="weight-chart">
      <canvas ref={canvasRef} aria-label="Exercise strength over time" role="img" />
    </div>
  );
}
